...
static void togglespecialtag(const Arg *arg);
static void movetospecialtag(const Arg *arg);
...
void
togglespecialtag(const Arg *arg) {
    static int showspecial = 0;           // Track whether the special tag is shown
    static unsigned int prevtags = 0;    // Store the previous active tagset

    showspecial = !showspecial;

    if (showspecial) {
        // Save the current tagset as the previous tagset
        prevtags = selmon->tagset[selmon->seltags];
        // Switch to the special tag (9th tag)
        ...
        if (selmon->tagset[selmon->seltags] != (1 << 8)) {
            // Update prevtags if the user has switched tags manually
            prevtags = selmon->tagset[selmon->seltags];
        }
        // Restore the previous tagset
        selmon->tagset[selmon->seltags] = prevtags;
    }

    focus(NULL);        // Update focus
    arrange(selmon);    // Re-arrange windows
}

void
movetospecialtag(const Arg *arg) {
    if (selmon->sel) {
        selmon->sel->tags = (1 << 8); // Assign the special tag (9th tag)
        arrange(selmon); // Re-arrange to update the layout
    }
}
